...
export class AcpCircuitBreaker {
  ...
  async execute<T>(fn: () => Promise<T>): Promise<T> {
    ...
    if (this._state === "half-open" && this.probing) {
      throw new CircuitHalfOpenError("Circuit half-open probe already in progress");
    }
  ...
  async executeWithStallTimeout<T>(
    ...
    let settled = false;
    let resolveRace: (value: { result?: T; stalled: boolean; error?: unknown }) => void;
    const racePromise = new Promise<{ result?: T; stalled: boolean; error?: unknown }>((resolve) => {
      resolveRace = resolve;
    });
...
export function killWithEscalation(proc: ChildProcess, escalationMs = 5000): void {

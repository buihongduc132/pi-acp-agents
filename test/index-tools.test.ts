...
it("registers exactly 9 tools (7 ACP core + 2 hooks policy)", () => { expect(tools.size).toBe(9); });
	...
	it("acp_spawn creates a session (absorbed acp_prompt spawn + acp_session_new)", async () => { const r = await exec("acp_spawn", { agent: "gemini", prompt: "hi", async: false }); expect(r.content[0].text).toBe("response"); });

...
## Purpose
Defines how entries in `agent_servers` config model **agent profiles** (named
persona + prompt + narrower goal + description) layered on top of a shared
**server** (transport: command/args/env/cwd). Covers the optional `description`
field that surfaces in listing surfaces for discovery, and the conceptual
server-vs-profile distinction.
## Requirements
